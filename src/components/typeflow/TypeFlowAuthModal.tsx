          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '0.6rem', paddingTop: '0.85rem', borderTop: '1px solid var(--tf-border)' }}>
              <button
                ...
                style={{ flex: 1, background: 'var(--tf-surface-elevated)', border: '1px solid var(--tf-border)', fontSize: '0.8rem', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}
              >
                <Download size={14} />
                <span>{isTr ? 'Verilerimi İndir (.json)' : 'Export Data (.json)'}</span>
              </button>
              <button
                type="button"
                className="tf-btn tf-btn-pushable"
                ...
                disabled={loading}
                style={{ flex: 1, background: 'rgba(239, 68, 68, 0.15)', border: '1px solid rgba(239, 68, 68, 0.4)', color: '#ef4444', fontSize: '0.8rem', fontWeight: 700, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}
              >
                <LogOut size={14} />
                <span>{isTr ? 'Oturumu Kapat' : 'Sign Out'}</span>
              </button>
            </div>
          </div>
        )}
          ...
          <div>
            ...
            {view === 'login' && (
              <form onSubmit={handleLogin}>
                ...
                >
                  <LogIn size={15} />
                  <span>{loading ? (isTr ? 'Giriş Yapılıyor...' : 'Signing In...') : (isTr ? 'Giriş Yap' : 'Sign In')}</span>
                </button>
              </form>
            )}
            ...
            {view === 'register' && (
              <form onSubmit={handleRegister}>
                ...
                >
                  <UserPlus size={15} />
                  <span>{loading ? (isTr ? 'Hesap Oluşturuluyor...' : 'Creating Account...') : (isTr ? 'Hesabımı Oluştur' : 'Create Account')}</span>
                </button>
              </form>
            )}
